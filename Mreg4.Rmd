---
title: "Wie findet man das passende Modell für die Mehrebenenregression?"
...
Wir schätzen also unser neuen Modell, und für die Cross-Level Interaktion müssen wir nur einen ganz regulaären Interaktionseffekt in der Formel spezifizieren. R wei dann von ganz alleine, dass die eine Variable auf Ebene 1 ist und die andere Variable auf Ebene 2.
...
Das bedeutet, das mit Zunahme der Korruption um einen Skalenpunkt verändert sich der Effekt von Bilung um den Faktor $`r fixef(mreg4)[8]`$. Je höher die Korruption ist, desto größer, negativer wird der Effekt von Bildung. Und umgekehrt: je weniger Korruption in einem Land herrscht, desto positiver wird der Effekt von Bildung auf politisches Vertrauen.

Da es sich bei Interaktionseffekten um bedingte Effekte handelt, dürfen wir auch den Haupteffekt von Bildung nur in Abhängigkeit der Korruptionsvariable interpretieren - der Haupteffetk von Bildung ist der Effekt den man beobachtet, wenn die Interaktionsvariable den Wert $0$ hat. Also: in einem ein Land, das sehr sauber ist, Bildung einen nicht signifikanten Effekt.  

Man kann auch sagen: in Ländern mit hoher Korruption sorgt Bildung hohe Bildung dafür, dass man den poltischen Eliten weniger Vertraut. In Ländern mit wenig Korruption wirkt Bildung dagegen positiv auf das Vertrauen.

# Schluss



Wichtig ist, dass man nicht beliebig die Ebene 1 Prädiktoren variieren lassen sollte. Nur wo es eine Relevanz für die Forschungsfrage hat und nur dann, wenn auch die Theorie eine begründete Hypothese zulässt, werden variierende Slopes im Modell spezifiziert.
...
```{r}
anova(mreg2, mreg3, refit=F)
anova(mreg0, mreg1, mreg2, mreg3, mreg4, refit=F, ddf="lme4")
```
