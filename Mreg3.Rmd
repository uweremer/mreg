...
- dass Sie ein eifaches Mehrebenenmodell als Formel ausdrücken können
...
Ergänzen wir das Modell nun noch um einen Prädiktor auf Ebene 2. Da wir als weitere Forschungsfrage wissen wollen, ob das Ausmaß an Korruption in einem Land den Effekt von Bildung beeinflussen kann, müssen eine Variable für Korruption auf Ebene 2 ergänzen.

Wir nehmen also den Korruptionsindex von Transparency International mit ins Model. Die Variable heißt `c_ticpi_2018` und sieht folgendermaßen aus:
...
Da es eine Ebene 2 Variable ist, kann sie nur den länderspezifischen Mittelwert der abhängigen Variable beeinflussen. Die länderspezifische Korruption muss also Teil der Gleichung des variierenden Intercepts $\beta_{0j}$ sein. Wir ergänzen dafür den Koeffizienten $\gamma_{01j}$ in der zweiten Zeile der Gleichung:
...
Wie wir bei den Fixed Effects sehen könnnen, hat also `c_ticpi_2018` einen kleinen aber - zumindest bei 10%iger Irrtumswahrscheinlichkeit - einen signifikanten positiven Effekt auf den variierenden Intercept. Also mit Zunahme des Korruptionsindex um einen Skalenpunkt steigt das durchschnittliche politische Vertrauen in einem Land um $0.01$.
