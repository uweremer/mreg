...
knitr::opts_chunk$set(eval=TRUE, echo = T, message = FALSE, warning = FALSE,
...
Eim Hinweis: Da bei den Makrovariablen Daten für zwei Länder fehlen, liegen nach diesem Ausschluss fehlender Werte nur Daten für gut 40.000 Befragte aus 25 Ländern vor. 
...
Das speichere ich im einem neuen Objekt und überschreibe danach die ursprünglichen Variablen 2 bis 5.
...
Die Form ist die selbe wie beim Nullmodell. Allerdings enthält die Formel nun auch die Ebene 1 Prädiktoren $x_1$ bis $x_4$.
...
Die anderen Determinanten haben einen positiven Effekt auf politisches Vertrauen. Da die wahrgenommene politische Responsivität eine andere Saklenbreite aufweist, können wir die Effektstärken jedoch nicht direkt miteinander vergleichen. 
...
Für Kroatien HR, leigt er $0.56$ Saklenpunkte *unter* diesem Wert. 
...
"Moment!" werden Sie jetzt sagen! Wie soll das den gehen! Wir haben doch nur Prädiktoren der Individualebene im Modell! Wie können individuelle Merkmale von Befragten Varianz zwischen den Ländern erklären?
...
Damit sind wir am Ende des dritten Videos. Sie sollten nunin der Lage sein ein erstes Mehrebenenmodell mit variierendem Intercept und Prädiktoren auf Ebene 1 schätzen und Interpretieren können.


# Variierender Intercept mit Prädiktoren auf L1 und L2

Ergänzen wir das Modell nun noch um einen Prädiktor auf Ebene 2. Da wir als weitere Forschungsfrage wissen wollen, ob das Ausmaß an Korruption in einem Land den Effekt von Bildung beeinflussen kann, müssen eine Varaible für Kooruption auf Ebene 2 ergänzen.

Wir nehmen also den Koouptionsindex von Transparency International mit ins Model. Die Variable heißt `c_ticpi_2018` und sieht folgendermaßen aus:
...
Da es eine Ebene 2 Variable ist, kann sie nur den länderpsezifischen Mittelwert der abhängigen Variable beeinflussen. Die länderspezifische Korruption muss also Teil der Gleichung des variierenden Intercepts $\beta_{0j}$ sein:
...
$gamma_{01j}$ ist also Regressionskoeffizient für den Einfluss der Variable `c_ticpi_2018` auf den variierenden Intercept.
...
Im R Befehl für die Mehrebeneregression müssen wir nicht explizit angeben, dass es sich um eine Kontextvariable handelt. Wir ergänzen sie einfach in der Formelm wie jede andere Variable auch.
...
Wie wir bei den Fixed Effects sehen könnne, hat also `c_ticpi_2018` einen kleinen aber - zumindest bei 10%iger Irrtumswahrscheinlichkeit - einen signifikanten positiven Effekt auf den variierenden Intercept. Also mit Zunahme des Korruptionsindex um einen Skalenpunnkt steigt das durchschnittliche politische Vertrauen in einem Land um $0.01$.

Da der Korruptionsindex eine Skala von 0 bis 100 ist, ist das nicht wenig. Sehr korruptionsbelastete Länder haben einen Korruptionsindex von unter 50. Am anderen Ende stehen Länder mit einem Wert von 85. Bei 35 Skalenpunkten Unterschied ergibt sich also für die abhängige Variable politisches Vertrauen einen Unterschied von $0.35$ Punkten.
...
Das konditional e$R^2$ erhöt sich minimal von $44.1$ auf $45.6$. 
