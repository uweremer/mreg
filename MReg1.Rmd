...
Wenn wir beispielsweise eine Befragte haben, die auf der X-Variable einen Wert von 7 hat, müssen wir vom Intercept ausgehend also 7 Skalenpunkte nach rechts und $7\times b$ nach oben, um den Vorhersagewert für $Y$ zu erhalten. Also von `r round(coef(fit)[1],1)` aus 7 nach rechts und `r 7*round(coef(fit)[2],1)` nach oben. 
...
ggplot(data=ess_c6,
  ...
  theme_light() +
  theme(plot.margin=unit(c(.12,2,0,2), "cm")) +
  annotation_custom(grob = grid::textGrob(label = "Intercept a", 
                                          hjust=1, 
                                          gp=gpar(col="blue", cex=.91)),
                    xmin = -1.5, 
                    xmax = -1.5, 
  ...
  geom_polygon(data=shape, aes(x=x, y=y), 
               alpha=.3, fill = 'darkorange') +
  annotation_custom(grob = grid::textGrob(label = "Slope b", 
                                          hjust=0, gp=gpar(col="darkorange", cex=1)),
                    xmin = 2.15, 
                    xmax = 2.15, 
                    ymin = y_lab1,
                    ymax = y_lab1) +
  annotation_custom(grob = linesGrob(gp=gpar(col="blue", lwd=2)), 
                    xmin = 0, 
                    xmax = 7, 
                    ymin = coef(fit)[1], 
                    ymax = coef(fit)[1]) +
  annotation_custom(grob = linesGrob(gp=gpar(col="darkorange", lwd=2)), 
                    xmin = 7, 
                    xmax = 7, 
                    ymin = coef(fit)[1], 
                    ymax = coef(fit)[1] + 7*coef(fit)[2]) +
  annotation_custom(grob = grid::textGrob(label = "7 x Slope b", 
                                          hjust=0, gp=gpar(col="darkorange", cex=1)),
                    xmin = 7.15, 
                    xmax = 7.15, 
                    ymin = 3,
                    ymax = 3) +
  annotation_custom(grob = grid::textGrob(label = bquote(hat(y)==.(round(coef(fit)[1] + 7*coef(fit)[2], 1))), 
                                          hjust=1, gp=gpar(col="red", cex=1)),
                    xmin = 7, 
                    xmax = 7, 
                    ymin = 5.4,
                    ymax = 5.4) 
...
<font color=darkorange>$`r round(coef(fit)[2],1)`$</font>
$\times 7=$
<font color=red>$`r round(coef(fit)[1] + 7*coef(fit)[2],1)`$</font>
</p>


Dann sind wir bei `r round(coef(fit)[1] + 7*coef(fit)[2],1)` als Vorhersagewert für Menschen mit einer Wirtschaftszufriedenheit  von 7 Punkten.
