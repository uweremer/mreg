...
Auf der einen Seite gibt es die Wirklichkeit, auf der anderen Seite, das statistische Modell, mit dem wir diese Wirklichkeit möglichst gut abbilden wollen.

In der empirisch-analytischen Wissenschaft sagen wir, im der Wirklichkeit gibt es einen datengenerierenden Prozesses (DGP datagenerating process).  Das ist einen Kette an Ursache/Wirkungszusammenhängen, an deren Ende das Phänomen steht, das wir erklären wollen. Also eine Ablauf an Mechansimen, der dazu führt, dass z.B. unsere Befragten den einen oder anderen Wert auf der Y-Variable - der abhängigen Variable erhalten.

Unsere Theorien versuchen diesen DGP zu beschreiben - zumindest einen für uns relevanten Ausschnitt davon. Die Theorie liefert uns die unabhängigen Variablen, die dafür verwantwortlich sein sollen, warum eine Befragte den ein oder anderen Wert auf der Y-Variable aufweist.
...
Immer wenn solche hierarschichen Daten vorliegen, ist die Mehrebenenregression das passende statistische Modell, dass den datengenerierenden Prozess am besten abbilden kann. 
...
Wir besschäftigen uns hier mit dem einfachen Fall von zwei Ebenen.

- die erste Ebene ist die Individualebene, hier finden wir unsere Befragten. Es ist die untereste Ebene, kurz: L1 für Level 1. Als Index nutzen wir das $i$.
...
Nehmen wir beispielsweise den ESS Datensatz, dann haben wir: 
...
Wir haben $`r length(ess[,1])`$ Individuen $i_{1,\dots`r length(ess[,1])`}$ auf Ebene 1 geclustert in $`r length(unique(ess[,'cntry']))`$ Klassen/Gruppen
$j_{1,\dots `r length(unique(ess[,'cntry']))`}$ auf Ebene 2.



Die MLM Ist notwendig wenn ausreichend Varianz auf L2 vorhanden. ICC Design Effect 
Wenn nicht, ist die MLM Identisch mit der OLS
Wenn vorhanden aber nicht MLM sondern OLS: Verstoß gegen Regressionsannahmen, Unterschätzte Standardfehler
...
Dafür stehen unterschiedliche Varianten zur Verfügung, wie man das Modell spezifiziert:
Bild Gelman
