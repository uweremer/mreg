...
In der empirisch-analytischen Wissenschaft haben wir die vereinfachende Vorstellung, dass es eine Art datengenerierenden Prozess gibt (im Englischen: data generating process, oder kurz DGP). Das ist die Vorstellung, dass die Phänomene, also die Daten, die wir messen (also z.B. das Einkommen einer Person, oder ihr politisches Vertrauen usw.) das Ende oder Resultat einer komplexen Kette an Ursache/Wirkungszusammenhängen sind. 

Ein sehr simples Beispiel: Die Geburt in einem reichen demokratischen Industrieland als Kind wohlhabender Eltern führt zu guter Bildung führt höherem Einkommen führt größerer Zufriedenheit mit der Wirtschaftslage und führt zu einem höheren politischen Vertrauen. Und man könnte sich noch viele Abzweigungen, Kreuzungen und bedingte Effekte in dieser Kette vorstellen.

Praktisch kann man gar nicht alles berücksichtigen - darum geht es auch in der Forschung nicht. Aber dieses Bild des DGP hilft uns, die Komplixtät der Wirklichkeit handhabbar zu machen
...
Und unsere Theorien versuchen diesen Ablauf, diesen DGP zu beschreiben. Nicht alles, aber den für eine spezifische Forschungsfrage relevanten Ausschnitt. Die Theorie liefert uns nämlich die Annahmen über die unabhängigen Variablen und die Mechanismen, die sie verbinden. Sie liefern Annahmen darüberm, warum eine Befragte den ein oder anderen Wert auf der Y-Variable aufweist.
...
Unsere datengenerierenden Prozesse führen also zu einer verschachtelten Struktur der Daten (nested data). Man spricht auch von hierarchischen Daten, oder von einer Mehrebenenstruktur.
...
Wir beschäftigen uns hier mit dem einfachen Fall von zwei Ebenen.
...
- die Kontextebene ist die obere Ebene oder L2. Diese Ebene sind die Länder oder Klassen. Als Index nutzen wir das $j$.

Wir haben $n$ Individuen $i_{1, ...n}$ auf Ebene 1 geclustert in $j$ Klassen/Gruppen
$j_{1,...m}$ auf Ebene 2.

Hier ein konkretes Beispiel anhand der Daten des European Social Survey, kurz ESS:
...
save_fig("V1_0_1.png", save)
```

Die Grafik zeigt die Anzahl der Befragten in den 28 teilnehmenden Ländern.

Die Mehrebenenstruktur in den Daten lässt sich folgendermaßen ausdrücken:
...
Die Ländervariable ist also die Gruppierungsvariable, in denen die Einheiten der unteren Ebene geclustert sein müssen.

Theoretisch lassen sich noch weitere Ebenen einfügen. Zum Beispiel könnten die Befragten auch in Bundesländer oder Regionen geclustert sein, und diese Länder wiederum in Nationalstaaten. Für die Videoserie bleiben wir aber beim einfachen Beispiel mit zwei Ebenen.


Hier kommt ein wichtiger Punkt: 

Dass ihre Daten durch das vorhandensein Gruppierungsvariablen eine Mehrebenenstruktur aufweisen, ist zwar eine notwendige, aber keine hinreichende Bedingung für eine Mehrebenenregression.

Denn es ist ja nicht zwingend so, dass diese Struktur auch empirisch in zu Unterschieden zwischen den Kontexteinheiten führt.   
...
Weiter Vortiel istm das wir die Wechselwirkungen zwischen den Ebenen explizit im Modell berücksichtigen können und in wenigen Maßzahlen auch kompakt beschreiben können. 
...
Das was wir in den Daten an Variation haben, und das ist ja das, was wir erklären wollen setzt sich aus zwei Quellen zusammen. 

Oder bildlich gesprochen: diese Unordnung in der Punktewolke setzt sich aus zwei Quellen zusammen:

Einmal die Variation die durch Eigenschaften und Merkmale der Individuen entsteht. Und einmal die Variation, die aus Eigenschaften des Kontextes entsteht, die auf alle Individuen gleich wirken, die zum gleichen Kontext gehören. Wir haben also zunächst zwei Varianzkomponente.

Mit Hilfe der Mehrebenenregression lässt sich ein Modell spezifizieren, dass Varianzkomponenten korrekt berücksichtig.

Starten wir bei einem klassischen OLS Regressionsmodell, hier ein einfaches bivariates Beispiel: Wir berücksichtigen *keine* Mehrebenenstruktur:

**HIER Bild Gelman entwickeln**

Es gibt ein Intercept und ein Steigungsparameter b

Und Hier das das einfache Mehrebenenmodell:

- Wir können den Intercept zwischen den Kontexteinheiten variieren lassen! Wir können also berücksichtigen, dass es unterschiedliche Durchschnittswerte der abhängigen Variable in den verschiedenen Kontexteinheiten gibt. *Varying Intercept Model*

- darüber hinaus, können wir auch den Steigungsparameter variieren lassen. Also, erlauben, dass der Zusammenhang zwischen abhängiger Variable und unabhängiger Variable in den Ländern unterschiedlich stark sein kann. *Varying Intercept, Varying Slope Model*

Und wir können tatsächlich auch prüfen, ob und wie stark Merkmale auf der Kontextebene dafür verantwortlich sind, das diese Unterschiede auftreten. Das wäre dann das Modell mit einer *Cross-Level Interaction*
...
Die Mehrebenenregression schätzt einmal ein Modell für die Individualebene. Im Unterschied zur einfachen OLS dürfen aber bestimmte Parameter zwischen den Kontexteinheiten variieren. Und dann schätzt die Mehrebenenregression gleichzeitig ein Modell auf Kontextebene, um diese Variation auch erfassen und ggf. zu erklären.

Wir können also prüfen: 
