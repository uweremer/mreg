...
save_fig("V1_0_2.png", save)
```

Hier sind die Befragten in Regionen (z.B. Bundesländern) geclustert, die dann wiederum Teil von Ländern (also Nationalstaaten) sind. 
...
Weiterer Vorteil ist, dass wir die Wechselwirkungen zwischen den Ebenen explizit im Modell berücksichtigen können und in wenigen Maßzahlen auch kompakt beschreiben können. 
...
Und einmal die Variation, die aus Eigenschaften des Kontextes entsteht, die auf alle Individuen gleich wirken, die zum gleichen Kontext gehören. Wir haben also zunächst zwei Varianzkomponenten. 
...
Starten wir bei einem klassischen OLS Regressionsmodell, hier ein einfaches bivariates Beispiel. Das kennen Sie. A
...
Die Mehrebenenregression schätzt ein Modell für die Individualebene, aber im Unterschied zur einfachen OLS dürfen aber bestimmte Parameter zwischen den Kontexteinheiten variieren. Die Mehrebenenregression schätzt aber gleichzeitig ein Modell auf Kontextebene, dass es erlaubt, diese Variation auch systematisch erfassen und ggf. zu erklären.
...
- Varying Intercept Modell (mit oder ohne Prädiktoren auf L1 und L2)
- (Varying Slope Modell, mit oder ohne Prädiktoren auf L2)
- Varying Intercept, Varying Slope Modell (mit oder ohne Prädiktoren auf L2)
- Varying Intercept, Varying Slope Modell mit Cross-Level Interaktion
...
Diese Modelle schauen wir uns in Teil 3 der Videoserie noch einmal genauer an und werden sie mit R selbst umsetzen und interpretieren.


# Aufgabe zur Wiederholung

- Überlegen Sie sich theoretisches ein Beispiel, bei dem der durchschnittliche Wert einer Variable auf Ebene 1 zwischen den Kontexteinheiten variiert. 
